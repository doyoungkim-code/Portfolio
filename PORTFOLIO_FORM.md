...
| 클라우드 서비스 개발·운영 (AWS 등) | 우대 | SSABREE를 AWS EC2에 배포, 현재도 운영 중 | ✅ 스킬·프로젝트에 반영 |
...
앱스토어·원스토어에 실제 출시, 100+ 유저 확보, **AWS EC2에 배포해 현재도 운영 중** (2026-08-28 확인).
...
3. **API 스펙 반복 수정** — Req/Res 상세 명세 + Swagger 문서화로 1차 리뷰 80%+ 통과

---

## 5. 프로젝트 2 · 번역의 민족 🏆 전국 3위
...
**한 줄**: 대학 강의 실시간 번역 서비스 — 강사의 한국어 발화를 즉시 영어 음성·자막으로 변환.
원문을 외부 서버로 보내지 않는 **완전 로컬 AI 추론**이 차별점 (서비스 특성 설명, 팀 전체 성과).
...
### 트러블슈팅 — 17GB 대용량 패키징 (골격만 확정, 세부는 TODO)
- 문제: 17GB AI 모델을 포함한 데스크톱 앱을 비개발자(강사)가 설치 가능한 단일 설치 파일로 만들어야 함.
  빌드 용량·시간, 설치 대상 PC의 권한·디스크 제약이 걸림.
- 접근: 3단계 패키징 파이프라인 정리, per-user 설치(관리자 권한 불필요), 설치 전 디스크 용량 사전 체크, 모델 다운로드 플로우 분리.
- `TODO`: 구체적 실패 사례·수치·해결 디테일을 사용자와 대화로 발굴해 보강할 것.

...
현재 `ssabree-1.jpg` · `translate-1.png` · `og.jpg`는 **데모 화면을 캡처해 자동 생성한 임시본**이다.

```
npm run dev       # dev 서버
npm run capture   # → ssabree-1.jpg, translate-1.png, og.jpg 재생성
```
...
| `og.jpg` | 카톡·슬랙·메일 링크 미리보기 (`index.html` og:image) | 1200×630 | 1페이지 캡처 (자동) |

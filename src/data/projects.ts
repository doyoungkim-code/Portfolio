...
export const project1 = {
  ...
  demo: {
    ...
    /* 챕터 전환 시 휴대폰 전체를 먼저 보여준 뒤, 기능이 일어나는 부분으로 확대 (논리 좌표).
       가로는 항상 휴대폰 전체 폭(베젤 포함 x 36~404)을 담아 글자가 좌우로 잘리지 않게 하고, 세로 위치만 챕터별로 옮긴다 */
    focus: {
      0: { x: 36, y: 200, w: 368, h: 540 }, /* 로그인 폼 */
      1: { x: 36, y: 20, w: 368, h: 540 },  /* 홈 상단: 인사말·D-Day·배너 */
      2: { x: 36, y: 110, w: 368, h: 540 }, /* 게시글·투표·댓글 */
      3: { x: 36, y: 110, w: 368, h: 540 }, /* 글쓰기 */
      4: { x: 36, y: 130, w: 368, h: 540 }, /* 모집글·지원 */
      5: { x: 36, y: 10, w: 368, h: 540 },  /* 나의 그룹 · 수락 헤드업 */
      6: { x: 36, y: 290, w: 368, h: 540 }, /* 채팅방: 최근 메시지 + 입력창까지 (화면 하단 기준) */
      7: { x: 36, y: 20, w: 368, h: 540 },  /* 알림함·마이페이지 */
    } as Record<number, FocusRect>,
    title: 'SSABREE TIME 데모 — 갤럭시 폰 2대 실시간 동기화',
    steps: [
      '앱 시작 — 스플래시 & Mattermost 인증 로그인',
      '홈 대시보드 — 인사말 · D-Day · 모집 배너 · 식단 · 게시판',
      '게시판 — Hot 게시글 · 투표(Poll) · 좋아요 · 댓글/대댓글',
      'AI 콘텐츠 검열 — 부적절한 글은 싸피봇(Gemini)이 블라인드',
      '그룹 매칭 — 스터디 모집글 확인 후 포트폴리오와 함께 지원',
      '실시간 수락 — 스터디장이 수락하면 내 폰에 FCM 푸시 도착',
      '실시간 쪽지 — WebSocket(STOMP) 채팅, 상대 메시지가 즉시 도착',
...
export const project2 = {
  ...
  demo: {
    ...
    steps: [
      ...
      '마이크 켜고 강의 시작',
      '발화가 끝날 때마다 ASR → NMT → 자막 → TTS 로 전달',
      '커서 스팟라이트와 판서가 수강자 화면에 동기화',
      '회선 품질 변화도 헤더 칩으로 표시 (P90 적응형 딜레이)',
      '일시정지 — 수강자 화면에 Paused 오버레이',
